...
def simulate_free_particle(n_trajectories, n_steps, dt, D, seed = 0):
	"""Simule x(t) pour une particule libre en 1D:
	dx = sqrt(2D) dW  (pas de force externe)
	"""
...
def print_summary(results):
	print("=== Résumé statistique (particule libre, diffusion 1D) ===")
	for D, res in results.items():
		t = res["t"]
		mean_t = res["mean"]
		var_t = res["var"]

		# Ajustement linéaire var(t) ~ a * t + b; on attend a = 2D
...
def plot_time_statistics(results):
	...
	ax_std.set_title("Écart-type sigma_x(t)")
	ax_std.set_xlabel("t")
...
def plot_distribution_at_time(results, t_index=None, bins=80):
	# Distribution P(x,t) à un instant donné et comparaison gaussienne.
	nD = len(results)
	...
	for i, (D, res) in enumerate(results.items()):
		...
		pdf = (1.0 / np.sqrt(2 * np.pi * sigma2)) * np.exp(-((x_grid - mu) ** 2) / (2 * sigma2)) if sigma2 > 0 else np.zeros_like(x_grid)
		ax.plot(x_grid, pdf, "r-", lw=2, label="Théorie gaussienne")

		ax.set_title(f"Distribution à t={t0:.2f}, D={D}")
		ax.set_xlabel("x")
		ax.set_ylabel("P(x,t)")
